...
numeroWaypoint = 0
tiempoAutonomia = 430 #tiempo autonomia total aprox 450s
counterCal = 2
sumaBlanco = 0.0
sumaCapturado = 0.0
...
def guardarDescanso():
    ...
    #     pass
    puntoResume = arrayWaypointsD[0].split(",")
...
def reanudarMision():
    ...
    try:
    # ----------------------------------LLAMADO A DRONE.PY------------------------------
        errorDespegue = estadoResume(remainingWp, float(puntoResume[0]), float(puntoResume[1]))
        # sleep(0.3)
        # FSM(1)
    # ----------------------------------------------------------------------------------
    except Exception as errorDespegue:
